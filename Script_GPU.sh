...
while ( ${cnt} <= ${cntmax} )
    ...
    if ( ${cnt} == 1 ) then
      ...
      gmx grompp -f step6.{$cnt}_equilibration.mdp -o step6.{$cnt}_equilibration.tpr -c step6.{$pcnt}_minimization.gro -r step6.0_minimization.gro -n index.ndx -p topol.top
	    ...
	    gmx mdrun -ntmpi 1 -ntomp 24 -gpu_id {$gpu_id} -pme gpu -bonded gpu  -nb gpu -update gpu -pin on -pinoffset {$num} -v -deffnm step6.{$cnt}_equilibration
    else
...
gmx mdrun -ntmpi 1 -ntomp 24 -gpu_id 0 -pme gpu -bonded gpu  -nb gpu -update gpu -pin on -pinoffset {$num} -v -deffnm ${system}

#gmx convert-tpr -s ${system}.tpr -extend 400000 -o ${system}1.tpr #extend ps

#gmx mdrun -ntmpi 1 -ntomp 24 -gpu_id 0 -pme gpu -bonded gpu -nb gpu -update gpu -pin on -pinoffset {$num} -v -deffnm ${system} -cpi ${system}.cpt -append -s ${system}1.tpr
