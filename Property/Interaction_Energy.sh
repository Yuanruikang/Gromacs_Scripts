...
type="PROT-MEMB"
  {
    ...
    echo q
  } |gmx make_ndx -f ../${name}.tpr -o inter_${type}.ndx -n ../index_pbc.ndx
  echo -e "${r_1}_MEMB" |gmx trjconv -s ../${name}.tpr  -f ../Traj_${name}_CenMEMB.xtc -n inter_${type}.ndx -o inter-${type}.xtc -b ${begin}  -e ${end}
  echo -e "${r_1}_MEMB" |gmx convert-tpr -s ../${name}.tpr -n inter_${type}.ndx -o inter-${type}.tpr
