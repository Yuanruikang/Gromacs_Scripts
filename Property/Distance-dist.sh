...
mkdir AA_distance
cd AA_distance
echo "SS3                      Average   Err.Est.       RMSD  Tot-Drift" >> AAdistance_${name}.log
while [ "$type" -lt $AA_amounts ]; do
  ...
  gmx analyze -f ${name}_r${type}_PO4_XYZ.xvg -dist r${type}_${name}_dist.xvg -b 400 | tail -2 >> AAdistance_${name}.log
  let "type++" 
done
rm -rf *#
